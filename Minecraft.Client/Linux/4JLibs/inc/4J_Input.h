// Linux-adapted copy of Windows64/4JLibs/inc/4J_Input.h.
//
// Identical to it apart from the two mouse-look entry points at the end of
// C_4JInput. Nothing else in the surface needs to change - it has no D3D/Win32-only
// types - which is why the Windows64 header was reused verbatim until mouse look
// needed an interface of its own. See Linux/4JLibs/inc/4J_Render.h and
// 4J_Storage.h for the same fork-only-when-the-signature-really-differs precedent.
//
// The mouse cannot be expressed through GetJoypadStick_RX/RY: a stick position
// means a turn RATE, which caps how fast the camera can spin no matter how fast
// the mouse moves, and which is sampled by more than one caller so it cannot be
// consumed on read. Mouse motion is a displacement, so it gets its own drain.
#pragma once
...
public:
	...
	// 4J-Linux: raw mouse look, pad 0 only. ConsumeMouseLook drains the motion
	// banked since the last call, in pixels (+X right, +Y up), and zeroes it - so
	// it must have exactly ONE caller (GameRenderer::render, once per rendered
	// frame). MouseMovedRecently is a side-effect-free "is the mouse in use" test
	// for the AFK/idle check, which would otherwise never see a mouse-only player.
	void				ConsumeMouseLook(float *pfPixelsX, float *pfPixelsY);
	bool				MouseMovedRecently(void);

	// 4J-Linux: hotbar selection, pad 0 only. Neither can go through the action /
	// button-bitmask layer: the mouse wheel is event-only (SDL never exposes it as
	// state to poll), and there is no absolute-slot action in MINECRAFT_ACTION_* at
	// all - Inventory::swapPaint is relative and one step at a time.
	//
	// Both drain on read and both are banked until then, because the caller is
	// Minecraft::tick(), which runs zero times in most frames above 20fps.
	int					ConsumeMouseWheel(void);        // notches, + = away from user
	int					ConsumeHotbarSlotRequest(void); // 0-8, or -1 for none
