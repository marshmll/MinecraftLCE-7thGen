...
// Linux-adapted copy of Windows64/4JLibs/inc/4J_Render.h. The vendor-supplied
// C4JRender interface has no source anywhere in the leak (only a prebuilt
// Windows .lib) - Minecraft.Client/Linux/LinuxExtras/LinuxRender.cpp provides
// a real body for every method here, against OpenGL.
//
// Only Initialise() changes shape versus Windows64: the original takes a
// cached D3D11 device/swapchain pair so Present() can call swapchain->Present()
// later. On Linux, presentation is SDL_GL_SwapWindow(window), so Initialise
// instead takes an opaque window handle (an SDL_Window*, cast to void* by the
// caller) that LinuxRender.cpp casts back internally. This header must not
// include <SDL2/SDL.h> - Minecraft.World/stdafx.h includes this path and
// Minecraft.World does not link SDL2. Every other signature is byte-for-byte
// identical to Windows64's; D3D11 types (ID3D11Buffer*, D3D11_RECT*, HRESULT,
// ...) already resolve to opaque/POD types via LinuxStubs.h/LinuxTypes.h.
...
public:
	...
	// Core
	void Initialise(void *pWindowHandle);
